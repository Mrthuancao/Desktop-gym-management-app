...
namespace DAO
{
    public class ThietBiDAO
    {
        ...
        {
            string query = "update Account set matb = @maThietBi, ngmua = @ngayMua, ngsd = @ngaySuDung, hanbaotri = @hanBaoTri, gia = @money, mahang = @maHang, maltb = @maLoaiThietBi, soluong = @soLuong where tenthietbi = @tenThietBi";
            object[] value = new object[] { tenThietBi, maThietBi, ngayMua, ngaySuDung, hanBaoTri, money, maHang, maLoaiThietBi, soLuong };
            DBConnect db = new DBConnect();
            return ((db.ExecuteNonQuery(query, value)) > 0);
        }
